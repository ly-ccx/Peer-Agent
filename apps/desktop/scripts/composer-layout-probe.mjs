...
try {
  ...
  for (const variant of ['home', 'compact']) for (const width of [280, 360, 760]) for (const long of [false, true]) for (const scale of [1, 1.5]) {
    ...
    await page.setContent(`<style>:root {font-size:${16 * scale}px; --composer-addon-height:1.75rem; --composer-addon-font-size:.8125rem; --ui-font-control:.8125rem; --ui-font-caption:.75rem; --space-2:8px; --space-3:12px; --space-4:16px; --space-5:20px; --space-6:24px;} *{box-sizing:border-box} body{font-family:Arial} ${css}</style>
      ...
      <div class="goal-panel goal-panel--docked"><button class="goal-panel-toggle"><span class="goal-panel-toggle-active"><span class="goal-panel-toggle-active-dot"></span><span class="goal-panel-toggle-active-text"><span id="title" class="goal-panel-toggle-active-title">${title}</span><span id="activity" class="goal-panel-current-activity">${long ? '正在执行自动化任务'.repeat(5) : '执行中'}</span></span><span class="goal-panel-toggle-active-progress">1/4</span></span><span>⌄</span></button></div>
