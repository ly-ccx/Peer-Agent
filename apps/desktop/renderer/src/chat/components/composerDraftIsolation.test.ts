...
test('new tasks can opt into worktree isolation from the draft composer', async () => {
  ...
  assert.match(surface, /composer-chrome-left[\s\S]*GoalPlanPanel[\s\S]*workspaceIsGit === true \?[\s\S]*composer-chrome-right[\s\S]*composer-env-status/);
  assert.match(surface, /composer-chrome-right[\s\S]*composer-env-status[\s\S]*<ComposerDraftControls/);
  assert.match(surface, /<ComposerDraftControls[\s\S]*chat-composer-toolbar[\s\S]*composer-workspace-dropdown/);
  assert.doesNotMatch(surface, /composer-env-status[\s\S]*composer-workspace-dropdown[\s\S]*<ComposerDraftControls/);
  assert.doesNotMatch(surface, /chat-composer-toolbar[\s\S]*composer-env-status/);
  assert.doesNotMatch(surface, /composer-worktree-toggle/);
  assert.match(surface, /composer-env-status/);
  assert.doesNotMatch(surface, /composer-env-capsule-dropdown/);
