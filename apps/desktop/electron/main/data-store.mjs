...
/** 数据根，默认 ~/.peer-agent，可用 PEER_AGENT_HOME 覆盖（多环境 / 测试隔离）。 */
...
/**
 * 一次性从旧 Electron userData 把业务数据搬到 ~/.peer-agent。
 *
  ...
  for (const [key, entry] of Object.entries(DATA_STORE_ENTRIES)) {
    ...
    if (existsSync(from) && !existsSync(to)) {
      try {
        ...
        console.warn(`[data-store] migrate failed for ${key}:`, error?.message ?? error);
      }
    }
  }
  if (done.length > 0) {
    console.log('[data-store] migrated from legacy userData:', done.join(', '));
  }
...
export function exportBundle(targetDir) {
  ...
  for (const entry of listEntries({ scope: 'portable' })) {
    ...
    try {
      ...
      console.warn(`[data-store] export failed for ${entry.key}:`, error?.message ?? error);
    }
  }
...
export function importBundle(sourceDir) {
  ...
  for (const entry of listEntries({ scope: 'portable' })) {
    ...
    try {
      ...
      console.warn(`[data-store] import failed for ${entry.key}:`, error?.message ?? error);
    }
  }
  return { imported };
}
