...
function requireHostPort(name, value) {
  if (typeof value !== 'function') {
    throw new TypeError(`Google OAuth requires a ${name} host port`);
  }
  return value;
}
...
async function exchangeCode({ code, verifier, fetchImpl }) {
  const fetchToken = requireHostPort('fetchImpl', fetchImpl);
  const res = await fetchToken(TOKEN_URL, {
    method: 'POST',
...
export async function refreshGoogleAccessToken(tokens, { fetchImpl } = {}) {
...
export function startGoogleBrowserLogin({ fetchImpl, openExternal } = {}) {
  const openBrowser = requireHostPort('openExternal', openExternal);
  ...
  server = http.createServer(async (req, res) => {
    ...
    try {
      const tokens = await exchangeCode({ code, verifier, fetchImpl });
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
  ...
  function onListening() {
    ...
    Promise.resolve(openBrowser(authorizeUrl.toString())).catch((err) => finish(rejectFn, err));
  }
