...
function requireQoderHostPort(name, value) {
  if (typeof value !== 'function') {
    throw new TypeError(`Qoder private stream requires the ${name} host port`);
  }
  return value;
}
...
async function sendQoderPreparedStream({
  ...
  const fetchHost = requireQoderHostPort('fetchWithRecovery', fetchWithRecovery);
  const consumeHost = requireQoderHostPort('consumeStream', consumeStream);
  const res = await fetchHost(preparedUrl, firstInit, {
    webContents,
    streamId,
    provider: 'qoder',
    model: requestBody.model_config.key,
  ...
  const streamResult = await consumeHost(res, webContents, streamId, trace, signal, {
...
/**
 * Execute the Qoder private stream through host-provided transport ports.
 * Provider request fields remain extensible because callers project the
 * canonical request shape before it reaches this adapter.
 *
 * @param {Record<string, any> & {
 *   fetchWithRecovery: Function,
...
export async function sendQoderPrivateStream({
  ...
  if (metadata) {
    return sendQoderPreparedStream({
      ...
      modelOptionValues,
      effort,
      fetchWithRecovery: fetchHost,
      consumeStream: consumeHost,
    });
  }
  ...
  let firstAttemptUsed = false;
  const res = await fetchHost(url, firstInit, {
    webContents,
    streamId,
    provider: 'qoder',
    model: body.model,
