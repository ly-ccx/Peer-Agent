...
export function renderAdaptivePlanningPrompt() {
  return joinPromptSections([
    ...
    '- L1 Micro-plan: single-axis low-risk change (one behavior / one contract). File count is not the depth signal. Keep a short 3–6 step plan in the turn. If there is any write or other side effect, record a machine-checkable success check.',
    '- L2 Auto Goal: multi-step feature/fix, cross-file work, multi-axis behavior, library contract, code generation, or serialization/config interaction — create formal GoalPlan via goal_create_plan with structured successCriteria and self-drive without waiting for plan approval.',
    ...
    'Upgrade when impact grows, a second independent behavior axis appears, verify/adjust needs many turns, or the same symptom returns after a failed cosmetic fix.',
