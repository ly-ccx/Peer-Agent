  ...
  return joinPromptSections([
    ...
    [
      ...
      bulletList([
        ...
        `Never narrate "writing" / "正在写入" / "开始写入" as if a file write is already in progress. Emit a real ${TOOL_NAMES.writeFile} or ${TOOL_NAMES.editFile} tool call first; only after the tool result may you claim the file was written.`,
        `For large documents, prefer chunked writes: create or replace with a bounded ${TOOL_NAMES.writeFile} (content hard limit 32KB UTF-8), then append/revise with multiple ${TOOL_NAMES.editFile} calls. Avoid one giant write payload that can stall mid-stream before the tool call lands.`,
      ]),
    ].join('\n'),
    [
      'Tool selection:',
