...
- Regression baseline (2026-09-26, `PeerAgent/dev/0.1.0`): on macOS, `pnpm build`, `pnpm typecheck`, `pnpm architecture:check`, and `pnpm test` pass. The older list of five desktop failures, the `goal.test.ts` type error, and the two `@keyframes` violations is stale on this line — do not treat those names as expected failures. Linux CI can still cancel timer-based desktop tests when the full suite runs in parallel (`cancelledByParent`); the CI workflow retries each failed file once. A failure that remains after that retry is a regression.
